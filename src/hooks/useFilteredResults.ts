...
export function useFilteredResults(
  ...
  const filteredPlaces = useMemo(() => {
    return places.filter((p) => {
      if (
        selectedVillageId &&
        normalize(p.villageId) !== normalize(selectedVillageId)
      ) {
        return false;
      }
      ...
      if (openNow && !mockOpenNow(p)) {
        return false;
      }

      if (nearMe) {
        if (!userLoc) return false;
        if (haversineKm(userLoc.lat, userLoc.lng, p.lat, p.lng) > 5) {
          return false;
        }
      }
  ...
  const filteredEvents = useMemo(() => {
    return events.filter((e) => {
      if (
        selectedVillageId &&
        normalize(e.villageId) !== normalize(selectedVillageId)
      ) {
        return false;
      }
      ...
      if (nearMe) {
        if (!userLoc) return false;
        if (haversineKm(userLoc.lat, userLoc.lng, e.lat, e.lng) > 5) {
          return false;
        }
      }
